...
def gen_surface_mesh(x_segments, z_segments):
    vertices = []
    indices = []
...
def main_game(gwindow):
    ...
    meshes_dict = {}
    for lod in range(5, 100, 1):
        instances = gen_instance_pos(3, 2*pi)
        #vertices, indices = gen_vertices()
        vertices, indices = gen_surface_mesh(lod, lod)
        len_idx = len(indices)
    ...
    while running:
        ...
        y_player = cam.camera_pos[1]
        print(y_player)

        lod = int(frame_count/20)
        if lod < 5:
            vaoobject = meshes_dict[5]
            vao = vaoobject[0]
            len_idx = vaoobject[1]
            render_instance(vao, len_idx)

        if 100 > lod >= 5:
            vaoobject = meshes_dict[lod]
            vao = vaoobject[0]
            len_idx = vaoobject[1]
            render_instance(vao, len_idx)
        else:
            vaoobject = meshes_dict[99]
            vao = vaoobject[0]
            len_idx = vaoobject[1]
            render_instance(vao, len_idx)
