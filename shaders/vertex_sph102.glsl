...
void main() {
    ...
    float height_factor = clamp(abs(pos.y / height), 0.0, 1.0); // Clamping the height factor to 0-1
    //vec3 colormap = vec3(-pos.y / height, 0.5, -pos.y / height); // Normalizing -pos.y by height for a color gradient
    //vec3 finalcolor = colormap * color.rgb; // Multiply the color by the colormap to get the final color
    vec3 finalcolor = mix(color.rgb, vec3(1.0, 1.0, 1.0), height_factor); // Mix the color with white based on the height factor
    vertexColor = vec4(finalcolor * ambient, color.a);
