...
def create_grid(rows, cols, offset_x, offset_z):
    ...
    for i in prange(rows):
        for j in prange(cols):
            x = j + offset_x
            z = i + offset_z
            y = round(sin(x/2 + sin(z/4)), 2)
            vertices.extend([x, y, z])

    # Generate indices
    for row in prange(rows - 1):
        for col in prange(cols - 1):
            start = row * cols + col
            indices.extend([start, start + 1, start + cols])
            indices.extend([start + 1, start + cols + 1, start + cols])

    print('\n\n vertices', vertices, rows, cols, offset_x, offset_z, '\n\n')

    return np.array(vertices, dtype=np.float32), np.array(indices, dtype=np.uint32)


@jit(nopython=True, cache=True)
def generate_chunks(pos_x, pos_z, chunk_size, grid_rows, grid_cols):
    ...
    for i in range(-2, 3, 1):
        for j in range(-2, 3, 1):
            offset_x = (chunk_size * (GRID_SIZE - 1) / 2) + j * chunk_size
            offset_z = (chunk_size * (GRID_SIZE - 1) / 2) + i * chunk_size
            # Generate chunk
            vertices, indices = create_grid(grid_rows, grid_cols, offset_x, offset_z)
            # Correctly calculating chunk UID for symmetric placement
            print('i', i, 'j', j)
            print('off x', offset_x, 'off z', offset_z)
            print('uid', chunk_uid, '\n\n')
...
def generate_chunks_old(pos_x, pos_z, chunk_size, center_offset, grid_rows, grid_cols):
    ...
    for i in prange(GRID_SIZE):  # Row in the 3x3 grid
        for j in prange(GRID_SIZE):  # Column in the 3x3 grid
            ...
            # Generate chunk
            vertices, indices = create_grid(grid_rows, grid_cols, offset_x, offset_z)
            chunk_uid = (j - HALF_GRID_SIZE) + (i - HALF_GRID_SIZE) * GRID_SIZE
            chunks_data.append((vertices, indices, chunk_uid))
...
@jit(nopython=True, cache=True)
def generate_chunks_olld(pos_x, pos_z, chunk_size,  center_offset, grid_rows, grid_cols):
...
def main_game(gwindow):
    ...
    pos_x, pos_z = 0, 0
    chunk_data = generate_chunks(pos_x, pos_z, CHUNK_SIZE, GRID_ROWS, GRID_COLS)
