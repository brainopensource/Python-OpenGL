...
from OpenGL.GL import (
    ...
    glDrawElementsInstanced, glUniform3f, GL_DYNAMIC_DRAW
    )
...
def gen_surface_mesh(x_segments, z_segments, x_interval, z_interval, time):
    vertices = []
    indices = []

    wave_amplitude = 0.5  # Height of the wave
    wave_frequency = 1.0  # How frequent the waves are
    wave_speed = 2.0      # Speed at which wave moves
    ...
    for i in range(x_segments + 1):
        for j in range(z_segments + 1):
            ...
            wave_phase = time * wave_speed
            y_wave = sin(wave_frequency * (x + wave_phase)) * cos(wave_frequency * (z + wave_phase)) * wave_amplitude

            y = 0.1 * (sin(x + cos(z)) * sin(z + cos(x))) + sin(x/2)*0.5 + sin(z/2) + y_wave  # Apply wave effect
...
def main_game(gwindow):
    ...
    while running:
        ...
        if int(frame_count_2) % 3 == 0:
            vertices, _ = gen_surface_mesh(level_of_detail, level_of_detail,
                                           [0, distanciamento], [0, distanciamento],
                                           start_time)
