...
def compute_normals(vertices, indices):
    ...
    for i in range(0, len(indices), 3):
        ...
        # Calculate normal
        normal = np.cross(vector1, vector2, axisa=0, axisb=0, axisc=0)
        normal = normal / np.linalg.norm(normal)  # Normalize the vector
        
        # Add the normal to each vertex's normal (for smooth shading)
        normals[indices[i]] += normal
        normals[indices[i+1]] += normal
        normals[indices[i+2]] += normal

    # Normalize all normals (for smooth shading)
...
def main_game(gwindow):
    ...
    vertices, indices, offsets, colors = gen_grid_instanced(rows, cols,
                                                            ...
                                                            spacing)
    #normal = compute_normals(vertices, indices)
    #print(normal)
    #print(normal.shape, vertices.shape, indices.shape, offsets.shape, colors.shape)
