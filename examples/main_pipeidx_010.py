...
from OpenGL.GL import (
    ...
    GL_ELEMENT_ARRAY_BUFFER, glDrawElements, GL_UNSIGNED_INT,
    )
...
def mesh_triangle_vertices(x, y):
    """
    x,y,z,r,g,b
    :return: np.array(dtype=np.float32)
    """
    verts = [-x ,   -y,    0.0, 0.0, 0.0, 0.0,
              x ,   -y,    0.0, 0.0, 1.0, 0.0,
             -x ,    y,    0.0, 0.0, 0.0, 1.0,
              x ,    y,    0.0, 1.0, 1.0, 1.0] #,
...
def mesh_triangle_indexes():
    tri_idx = [0, 1, 2,
               1, 2, 3,
               ]
    return np.array(tri_idx, dtype=np.uint32)
...
def main_loop(awindow):
    ...
    vtx_array = mesh_triangle_vertices(0.5, 0.5)
    idx_array = mesh_triangle_indexes()
    len_idx = len(idx_array)
    initialize_object(vtx_array, idx_array)
