...
def create_grid(rows, cols, offsetx=0, offsetz=0):
    ...
    for i in prange(rows):
        for j in prange(cols):
            x = int(j + offsetx)
            z = int(i + offsetz)  # Ensure that we don't go out of bounds for z_vals
            y = round(sin(x)/2, 2)
...
@jit(nopython=True, cache=True)
def generate_chunks(player_position, chunk_size,  center_offset, grid_rows, grid_cols):

    # Calculate the base offset from the player's position
    base_offset_x = player_position[0] - center_offset
    base_offset_z = player_position[2] - center_offset
...
def create_world_chunks(chunks_data):
    preprocessed_chunks = {}
    for vertices, indices, chunk_uid in chunks_data:
        vao, vbo, ebo, len_idx = create_chunk_objects(vertices, indices)
        preprocessed_chunks[chunk_uid] = (vao, vbo, ebo, len_idx)
...
def main_game(gwindow):
    ...
    chunks_data = generate_chunks(position, CHUNK_SIZE, GRID_CENTER, GRID_ROWS, GRID_COLS)
